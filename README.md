...
**プレビュー**: [YoBi - AIショッピングアシスタント](https://the-tsukasa.github.io/AI-Shopping-Assistant-MVC/public/)  
...
## 🚀 インストール方法  
1. **ファイルのダウンロード**: 圧縮ファイルをダウンロードし、ローカル環境に解凍してください。  
2. **プロジェクトを開く**: VS Code を開き、解凍したフォルダを開きます。  
3. **依存関係をインストール**: ターミナルで以下のコマンドを実行：  
   ```sh
   npm install```  
4. **サーバーを起動**: ターミナルで以下のコマンドを実行：  
   ```sh
   node server.js```  
